    ...
    if hasattr(orig_mod, "PLUGIN_CLASS") and orig_mod.PLUGIN_CLASS:
        ...
        if not getattr(PLUGIN_CLASS, "display_name", None):
            try:
                ...
                log_debug(
                    "[cortex.llm_engine.selenium_gemini] Set fallback display_name for PLUGIN_CLASS"
                )
            except Exception:
                log_warning(
                    "[cortex.llm_engine.selenium_gemini] Failed to set fallback display_name"
                )
    else:
        log_warning(
            "[cortex.llm_engine.selenium_gemini] Original module found but PLUGIN_CLASS missing or None"
        )
except ModuleNotFoundError:
    log_warning(
        "[cortex.llm_engine.selenium_gemini] Original llm_engines.selenium_gemini module not found - this is a lightweight migration shim"
    )
except Exception as e:
    log_error(
        f"[cortex.llm_engine.selenium_gemini] Unexpected error importing original engine: {e}"
    )
