...
class ManualAIPlugin(AIPluginBase):
        ...
        try:
            ...
            if loop and loop.is_running():
                ...
                try:
                    ...
                    log_warning(f"[manual] Could not init message map table synchronously: {e}")
        except Exception as e:
    ...
    async def handle_incoming_message(self, bot, message, prompt):
        ...
        try:
            ...
            log_debug(f"[manual] core.config.get_trainer_id callable: {_config.get_trainer_id}")
        ...
        if not trainer_id:
            try:
                ...
                log_debug(f"[manual] fallback trainer id resolved via config.get_trainer_ids(): {trainer_id}")
            except Exception:
                pass
        if not trainer_id:
            log_warning("[manual] Missing trainer ID for telegram_bot; proceeding with fallback (test/limited env)")
        ...
        try:
            ...
            sent_heading = await _safe_send(bot, trainer_id, "\U0001f4e6 *Generated JSON prompt:*", parse_mode=ParseMode.MARKDOWN)
            log_debug(f"[manual] sent heading to trainer (trainer_id={trainer_id}) -> {repr(sent_heading)}")
            last_sent = None
            for i in range(0, len(prompt_json), 4000):
                chunk = prompt_json[i:i+4000]
                sent_chunk = await _safe_send(bot, trainer_id, f"```json\n{chunk}\n```", parse_mode=ParseMode.MARKDOWN)
                ...
                if bot is not None:
                    try:
                        ...
                        log_debug(f"[manual] forward_message result -> {repr(forwarded)}")
                    except Exception as forward_exc:
                        log_warning(f"[manual] forward_message failed, sending fallback link: {forward_exc}")
                        fallback_sent = await _safe_send(bot, trainer_id, f"(original message from chat {message.chat_id} id {getattr(message, 'message_id', 'unknown')})")
                ...
                if forwarded and getattr(forwarded, 'message_id', None) is not None:
                    ...
                    log_debug(f"[manual] using forwarded message id for mapping: {trainer_mid}")
                    await self.track_message(trainer_mid, message.chat_id, getattr(message, 'message_id', None))
                elif last_sent and getattr(last_sent, 'message_id', None) is not None:
                    trainer_mid = getattr(last_sent, 'message_id', None)
                    log_debug(f"[manual] using last_sent message id for mapping: {trainer_mid}")
                    await self.track_message(trainer_mid, message.chat_id, getattr(message, 'message_id', None))
                else:
