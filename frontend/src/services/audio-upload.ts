 ...
 * default engine (see services/audio-stream.ts). Token-gated on the backend
 * like the rest of the auth surface (see lib/api-token.ts).
 */
...
export async function transcribeClip(blob: Blob, signal?: AbortSignal): Promise<string> {
  ...
  const resp = await fetch(withApiToken('/api/audio/upload'), { method: 'POST', body: form, signal })
