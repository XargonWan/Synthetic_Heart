...
**Status:** known — not yet implemented (deliberately deferred; `run_actions` is a CRITICAL hot path with 45 upstream callers).
**Notes:** Design for the fix — add an opt-in `deliver_to_llm: True` convention to a plugin's `execute_action` result dict; `run_actions` captures such results into `action_outputs` (additive `elif` next to the terminal branch, so existing actions are untouched), and the existing generic `request_llm_delivery` voices them. **Interaction to handle:** once `recall_last_dream` triggers a delivery follow-up it *does* produce a reply, so it must also be reconciled with the missing-reply corrector (add it to `USER_OUTPUT_ACTION_TYPES`, or suppress the corrector when `run_actions` returned non-empty `action_outputs`) — otherwise the corrector and the delivery both fire and you get a double follow-up. This is why it's a 3-part change (capture + plugin flag + corrector reconciliation), not a one-liner.
