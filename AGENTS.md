...
| Mineflayer bridge (Node.js) | `plugins/rift_vessel/minecraft/minecraft_bridge_minimal.js` |
...
2. **No diary during a session.** Events accumulate in an in-DB `experience_buffer` on `vessel_sessions`; a **single** autobiographical "lived experience" diary entry is written **only at end-of-session** — explicit logout (`/vessel logout` CLI or the `vessel_disconnect` action) OR `VESSEL_SESSION_COOLDOWN_SEC` (default 3600 s) of inactivity, detected by the interface scheduler calling `close_expired_sessions`, OR **connector disconnection** (the client/bridge dropped) held for `VESSEL_DISCONNECT_GRACE_SEC` (default 30 s). The disconnection sweep runs **every scheduler tick** (`interface/vessel_interface.py::_close_disconnected_sessions`): for each active-session world it probes the connector's `is_connected` and, once the world stays disconnected past the grace window, force-closes the session (`end_sessions_for_environment(reason="disconnected")`) so `has_active_session()` flips false. This stops the accumulation of will-beat perceptions and lifts the message-queue deprioritisation — without it a dropped client would leave the session `active` for the full 3600 s cooldown, generating beats that block the flow. The Minecraft connector detects the drop structurally: `MinecraftConnector._poll_loop` counts consecutive `/events` poll failures and, after `_MAX_POLL_FAILURES` (5, ≈5 s), sets `self._connected = False` so `is_connected` reports the loss.
...
| `VESSEL_AUTONOMY_ENABLED` | Enable autonomous Rift Vessel play — both the slow will beat and the fast motor tick (default `False`) |
...
| `VESSEL_SP_LOW_OXYGEN` | Oxygen threshold below which the drowning reflex surfaces the body (default 200). **In AIR TICKS** (mineflayer `oxygenLevel`, vanilla max ~300 ≈ 15s), NOT the 0..20 hearts scale |
