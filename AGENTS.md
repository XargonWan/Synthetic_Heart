...
| `vessel_diary` | `core/db.py` / `init-db.sql` | Rift Vessel end-of-session lived-experience diary (`session_id`, `interface_path`, `environment`, `summary` LONGTEXT, `moments_count`, `reason`, `created_at`). Written by the chunked background compactor (`core/vessel_diary_compactor.py`) — **separate from the shared `ai_diary`** so in-world telemetry never pollutes ordinary Fast-Lane prompts |
...
| `VESSEL_DIARY_COMPACTION_ENABLED` | Enable the end-of-session lived-experience compaction into the dedicated `vessel_diary` table (default `True`). The experience buffer is summarised in chunks by `core/vessel_diary_compactor.py` on the vessel-scope Cortex, in a background task — **never** written to the shared `ai_diary` (which would pollute every non-vessel Fast-Lane prompt). Fully fail-safe: any LLM error degrades to a deterministic plain-text join |
