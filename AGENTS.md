...
In addition to Grillo, an **Agent plugin** can be implemented to give the SyntH a controlled "hand" to perform external tasks (e.g., shell commands, scheduled tasks) under policy-managed approval modes. The Agent plugin is intentionally **cortex-agnostic** and will attach to whichever active engine is configured for the selected cortex; it enforces safety via configurable approval modes (always_approve, whitelist, always_ask, disabled) and container-aware defaults. In the WebUI, the Components tab now requires selecting a *Cortex* kind (``llm``, ``live``, or ``agent``) first, and then choosing the specific engine for that cortex (for example, `selenium_chatgpt` or `gemini_live`). Currently, `gemini_live` is provided as the first *live* engine prototype.
