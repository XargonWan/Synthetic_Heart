...
- *Known false positives* (don't re-investigate): `grillo_compactor` extract_json tuple overload, Iris/Auris TypedDict capability dicts, discord `disconnect(force)` stub mismatch, `openai_api_server` (formerly `ollama_compat_server`) payload value-union subscript, `variables_engine` guarded casts, `models.py` hasattr-guarded isoformat.
...
**Status:** partially fixed.
**Notes:** The order-dependent pytest failures observed on `2026-05-06` in `tests/test_ai_diary_pool_behavior.py`, `tests/test_current_chat_history.py`, `tests/test_grillo_observer.py`, `tests/test_iris.py`, `tests/test_mobile_chat_behavior.py`, `tests/test_openai_api_server.py`, `tests/test_send_message_no_ws.py`, and `tests/test_vox_plugin.py` were fixed on `2026-05-07`. When working on a focused feature, still run the mandatory repo-wide commands for signal, but treat repo-wide Ruff failures as unrelated debt unless your slice touches those files. Use scoped lint/type checks on touched files plus targeted pytest around the modified area when the global lint pass is still dirty.
...
### Harmony (multi-modal) Iris returned no image description on Telegram — missing per-subsystem vision model  <!-- 2026-07-08 -->
**Symptom:** Images sent via Telegram were never described ("iris ha fallito a vedere le immagini su telegram"); the interface showed the unviewable-media placeholder. No exception surfaced because the bridge swallows errors (`except Exception: return None`).
**Location:** `core/external_endpoints/bridges/iris_bridge.py` (`ExternalIrisEngine.describe_image`).
**Status:** fixed (2026-07-08).
**Notes:** Same class of bug as the 2026-07-05 Harmony STT/TTS entry above, but for **Iris (vision)**. The active Iris engine `harmonyai` (a multi-modal Harmony endpoint) was being called with model `voicefixer` (an audio_conversion model with NO vision capability) sourced from the global `IRIS_DEFAULT_MODEL` (`plugins/iris_plugin.describe_media` passes `kwargs["model"]`), so the Harmony adapter's `describe_image` failed and the bridge returned `None`. **Fix (code):** `iris_bridge.py` now resolves a dedicated `extra_config.iris_model` as the endpoint's authoritative vision model — `vision_model = extra.get("iris_model") or model or self._endpoint.default_model` — overriding the mis-set global `IRIS_DEFAULT_MODEL`. Mirrors the `stt_model` (Auris) / `tts_model` (Vox) per-subsystem pattern. The silent `except Exception: return None` now also logs a warning (`[iris_bridge:<name>] describe_image failed (model=…, mime=…): <exc>`) to make future failures visible. **Fix (config):** set `harmonyai` `extra_config.iris_model = "gemma4-meromero-26b-a4b"` (a harmonyai vision-capable model, also the cortex default so already loaded) directly in live Postgres (`UPDATE external_endpoints SET extra_config = … WHERE name = 'harmonyai'`, all sibling keys preserved). **Lesson:** every media subsystem on a multi-modal endpoint needs its own model key in `extra_config` — the endpoint `default_model` / global `*_DEFAULT_MODEL` is the text/cortex model and is wrong for vision/STT/TTS. Registration is startup-only (see the note directly below), so this required `docker cp iris_bridge.py` + `docker restart synth`. Verified end-to-end at the bridge level: `IRIS_REGISTRY.load_engine("harmonyai").describe_image(<blue.png>)` returned `IrisResult(description='The dominant color of this image is blue.')`. **Testing gotcha (§12):** an isolated `docker exec .../python` process reads `ACTIVE_IRIS_ENGINE='disabled'` and empty config, so plugin-level tests fail — test the bridge/engine directly with an explicit `load_engine(...)`, or hit the running app's `GET /api/components`.
