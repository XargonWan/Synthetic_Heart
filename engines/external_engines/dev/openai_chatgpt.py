...
def get_interface_limits() -> dict:
    ...
    log_info(
        f"[openai_chatgpt] Interface limits for {model_name}: max_prompt_chars={limits['max_prompt_chars']}"
    )
    return limits
...
class OpenAIPlugin(AIPluginBase):
    ...
    def set_current_model(self, name):
        ...
        log_debug(
            f"[openai] Active model updated: {name} (max_prompt_chars: {max_chars})"
        )
            ...
            if bot and message:
                ...
                if interface_to_llm is None:
                    await bot.send_message(
                        chat_id=message.chat_id, text="⚠️ LLM response error."
                    )
                else:
