...
@pytest.mark.asyncio
async def test_cortex_response_send_dedupe(monkeypatch):
    sent = []
    ...
    res1 = await cortex_response_send("bot", 123, "Hello there")
    assert res1 == "ok"

    # Second identical send within dedupe window should be suppressed
    res2 = await cortex_response_send("bot", 123, "Hello there")
    assert res2 is None
