...
def test_rei_persona_has_blink_defaults():
    ...
    assert "close_ms" in blink and "hold_ms" in blink and "open_ms" in blink, (
        "blink timing fields missing"
    )
    assert isinstance(blink.get("intensity", None), (int, float)), (
        "blink intensity missing or invalid"
    )
