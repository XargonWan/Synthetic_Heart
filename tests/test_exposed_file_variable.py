...
def test_upload_and_download_exposed_file(tmp_path):
    ...
    assert 'attachment; filename="original.txt"' in r2.headers.get('content-disposition', '')
...
def test_upload_to_readonly_refused(tmp_path):
    ...
    register_exposed_var(key, label='ReadOnly File', default='', ui_type='file', readonly=True)
