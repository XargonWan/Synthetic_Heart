...
async def test_request_selective_correction_includes_message_in_context(monkeypatch):
    ...
    async def fake_run_corrector_middleware(text, bot=None, context=None, chat_id=None, thread_id=None):
    ...
    failed_actions = [
        {
            ...
            'errors': ["Unsupported type 'message_send' - no plugin or interface found to handle it"]
        }
    ]
    successful_actions = [
    ...
    # Call the helper
    await _request_selective_correction(failed_actions=failed_actions, successful_actions=successful_actions, bot=None, context={'interface': 'telegram'}, original_message=original_message)
