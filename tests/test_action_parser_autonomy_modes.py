...
async def test_whitelisted_mode_enforces_allowed_list(monkeypatch):
    ...
    await config_registry.set_value('AUTONOMY_ALLOWED_ACTIONS', ['create_personal_diary_entry'])
    ...
    ap.get_supported_action_types = lambda: set(["create_personal_diary_entry", "message_telegram_bot"])
    # No plugin for message_telegram_bot in our fake plugin list -> will be blocked as unsupported
    res2 = await ap.run_actions([action2], context={}, bot=None, original_message=original_message)
