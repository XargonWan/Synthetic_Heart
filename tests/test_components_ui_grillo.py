...
async def test_run_grillo_via_webui_api(monkeypatch):
                    ...
                    "clusters": [
                        {
                            "cluster_id": 1,
                            "should_compact": True,
                            "summary": "Mountains memory",
                            "summary_chars": 16,
                            "tags": ["hike"],
                            "feeling": "calm",
                            "source_ids": [501],
                            "confidence": "high",
                            "justification": "same theme",
                        }
                    ]
                }
            )
    ...
    try:
        res = await plugin_instance.run_action(
            "compact_now", {"cycles": 1, "dry_run": True, "marker": "test-marker-1"}
        )
...
async def test_run_action_emits_logs(monkeypatch):
    ...
    class DummyCursor:
        ...
        async def fetchall(self):
            return [
                {
                    "id": 601,
                    "content": "Walk in park",
                    "tags": json.dumps(["park"]),
    ...
    try:
        await plugin_instance.run_action(
            "compact_now", {"cycles": 1, "dry_run": True, "marker": "log-marker-99"}
        )
        assert any("run_action called" in m for m in logged), (
            f"Expected 'run_action called' in logs: {logged}"
        )
        assert any("log-marker-99" in m for m in logged), (
            f"Expected marker in logs: {logged}"
        )
    finally:
        if prev is None:
            del PLUGIN_REGISTRY["grillo_compactor"]
