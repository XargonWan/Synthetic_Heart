...
async def test_create_action_exec_creates_table_if_missing(monkeypatch):
    ...
    async def fake_get_conn_ctx():
        if call['n'] == 0:
            call['n'] += 1
            raise Exception("(1146, \"Table 'synth.grillo_action_execs' doesn't exist\")")
        return FakeConn()
    ...
    monkeypatch.setattr(GrilloPlugin, '_ensure_action_execs_table', classmethod(lambda cls: fake_ensure()))
