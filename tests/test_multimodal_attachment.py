...
class TestTelegramStickerExtraction:
    ...
    @pytest.mark.asyncio
    async def test_reply_to_sticker_fallback_without_promotion(self):
        """Test that a sticker on reply_to_message is extracted even when the
        current message has no sticker attribute (e.g. promotion failed)."""
        mock_bot = AsyncMock()
        mock_file = AsyncMock()
        mock_file.download_as_bytearray = AsyncMock(
            return_value=bytearray(b"fake_sticker_data")
        ...
        mock_reply.sticker = mock_sticker
        ...
        assert attachments[0]["media_metadata"]["emoji"] == "🦖"
        assert attachments[0]["media_metadata"]["set_name"] == "godzilla_stickers"
        mock_bot.get_file.assert_called_once_with("sticker_file_id")

    @pytest.mark.asyncio
    async def test_reply_to_photo_fallback_without_promotion(self):
        """Test that a photo on reply_to_message is extracted even when the
        current message has no photo attribute."""
        mock_bot = AsyncMock()
        mock_file = AsyncMock()
        mock_file.download_as_bytearray = AsyncMock(
            return_value=bytearray(b"fake_photo_data")
        )
        mock_bot.get_file = AsyncMock(return_value=mock_file)
        ...
        mock_reply = MagicMock()
        mock_reply.photo = [mock_photo]
        mock_reply.document = None
        mock_reply.sticker = None
        ...
        assert len(attachments) == 1
        assert attachments[0]["mime_type"] == "image/jpeg"
        assert attachments[0]["filename"] == "photo_photo_unique_id.jpg"
        assert attachments[0]["data"] == encode_bytes_to_base64(b"fake_photo_data")
        mock_bot.get_file.assert_called_once_with("photo_file_id")
