...
import core.persona_manager as pm  # ensure config keys are registered
...
async def test_llm_action_safe_false_blocked_by_default(monkeypatch):
    ...
    action = {"type": "create_personal_diary_entry", "payload": {"content": "x"}, "safe": False}

    import core.action_parser as ap
    ap._ACTION_PLUGINS = []
    ap.get_supported_action_types = lambda: set(["create_personal_diary_entry"])

    res = await run_actions([action], context={}, bot=None, original_message=original_message)

    # Should not execute (blocked/treated as proposal)
    assert res['processed'] == []
    assert len(res['failed_actions']) == 1
    assert 'unsafe' in res['failed_actions'][0]['errors'][0].lower() or 'proposal' in res['failed_actions'][0]['errors'][0].lower()
...
async def test_llm_action_safe_true_blocked_in_suggest_mode(monkeypatch):
    ...
    action = {"type": "create_personal_diary_entry", "payload": {"content": "x"}, "safe": True}
