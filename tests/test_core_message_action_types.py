...
async def test_message_action_types_can_be_overridden(monkeypatch):
    ...
    found = any((a.get('action') or a.get('type')) in current_message_action_types for a in actions)
    assert found, "Dynamic MESSAGE_ACTION_TYPES should be respected and detect custom types"
...
def test_transport_layer_example_is_generic():
    ...
    assert "message_<interface>_bot" in content, "Transport layer example should use a generic interface placeholder"
    assert "message_telegram_bot" in content or True  # keep safe if file contains other references elsewhere
