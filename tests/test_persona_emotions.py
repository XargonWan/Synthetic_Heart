def test_rei_persona_has_emotions_list():
    ...
    data = json.loads((root / 'skins' / 'Rei' / 'persona.json').read_text(encoding='utf-8'))
    ...
    assert expected.issubset(keys), 'persona.emotions keys must include canonical set (Ekman 6 + neutral + relaxed)'
