...
async def test_run_action_marker_filters(monkeypatch):
    ...
    res = await p.run_action('compact_now', payload={"cycles":1, "dry_run":True, "marker":"food"})
    assert res.get('status') == 'ok'

    # Retrieve queries captured by the cursor used during execution
    q = conn_instance.cursor().queries
    assert any('ai_diary' in sql and 'JSON_CONTAINS' in sql for sql, params in q), f"Expected ai_diary+JSON_CONTAINS in queries: {q}"
