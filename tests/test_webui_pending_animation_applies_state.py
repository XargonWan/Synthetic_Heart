...
def test_webui_pending_animation_applies_state():
    ...
    assert (
        "const st = last.animation_state || window.__synth_current_animation_state || null"
        in html
    ), "pending animation_state forwarding not present in template"
    assert (
        "animationHandler.applyAnimationState(st)" in html
        or "animationHandler.applyAnimationState(last.animation_state)" in html
    ), "applyAnimationState call not present for pending commands"
