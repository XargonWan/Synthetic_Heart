...
from core.db import get_db_connection, release_db_connection, execute_query
...
async def test_schedule_message():
    ...
    log_info(f"[test] Scheduling event for: {scheduled_time.strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    try:
        ...
        cursor.execute("SELECT id, type, scheduled_at, delivered FROM scheduled_events WHERE id = %s", (event_id,))
        row = cursor.fetchone()
        if row:
            log_info(f"[test] ✅ Event verified in DB: {row}")
        else:
            log_error(f"[test] ❌ Event NOT found in DB after creation!")
    ...
    log_info("[test] 📌 Now the scheduler should pick this up and deliver it to the LLM")
    log_info("[test] 📌 Watch the logs: tail -f logs/dev/synth.log | grep -E 'schedule_message|event_reminder|action_parser'")
