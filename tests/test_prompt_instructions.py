...
def test_unminified_chat_instruction_mentions_direct_reference():
    ...
    assert "refer to its author" in src_low, "Instruction to reference the author in a generic way is missing"
    assert "avoid vague or impersonal" in src_low, "Instruction to avoid vague phrasing is missing"
    # Ensure no leftover Italian fragments remain
    assert "ho visto" not in src_low and "qualcuno" not in src_low, "Found Italian text in unminified chat instruction; should be English only"