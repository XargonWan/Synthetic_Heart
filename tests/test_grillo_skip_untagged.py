...
async def test_skip_leading_untagged_candidates(monkeypatch):
    ...
    class DummyCursor:
        ...
        async def fetchall(self):
            return [
                {"id": 701, "content": "No tags here 1", "tags": None, "timestamp": "2020-01-01"},
                {"id": 702, "content": "No tags here 2", "tags": None, "timestamp": "2020-01-02"},
                {"id": 703, "content": "Tagged memory A", "tags": json.dumps(["a"]), "timestamp": "2020-01-03"},
                {"id": 704, "content": "Tagged memory B", "tags": json.dumps(["b"]), "timestamp": "2020-01-04"},
            ]
    ...
    class FakeEngine:
        async def generate_response(self, prompt):
            return json.dumps({"clusters": [{"cluster_id":1, "should_compact": True, "summary": "Tagged summary", "summary_chars": 14, "tags":["a"], "feeling":"ok", "source_ids":[703,704], "confidence":"high", "justification":"same"}]})
    ...
    assert any('Skipping 2 leading untagged candidate(s)' in m for m in logged), f"Expected skip log in: {logged}"
    # Ensure the dry-run results are present
    assert res.get('dry_run') is True
    assert len(res.get('results', [])) == 1
    assert res['results'][0]['status'] in ('ok', 'persisted', 'too_small', 'skipped_not_short_enough')
