...
def test_resolve_from_forward_metadata():
    ...
    reply = SimpleNamespace(message_id=12, reply_to_message=None, text=None, forward_from_chat=fchat, forward_from_message_id=555)
    import asyncio
    chat_id, msg_id = asyncio.run(telegram_bot._resolve_original_from_reply(reply))
...
def test_resolve_from_textual_fallback():
    reply = SimpleNamespace(message_id=13, reply_to_message=None, text='(original message from chat -4000 id 777)')
    import asyncio
    chat_id, msg_id = asyncio.run(telegram_bot._resolve_original_from_reply(reply))
...
def test_manual_tracks_last_sent_chunk(monkeypatch):
    ...
    message = SimpleNamespace(chat_id=123, message_id=456, from_user=SimpleNamespace(id=1, username='u'))
