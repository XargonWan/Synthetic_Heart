def test_archive_module_and_delegation():
    ...
    assert ('export function createArchiveModal' in mtxt) or ('export default' in mtxt and 'createArchiveModal' in mtxt), 'createArchiveModal API not found in archive-window.mjs'
    ...
    for legacy_marker in ['panel.id = \'archive-panel\'', "#archive-header", "archive-minimize", "archive-list", "archive-footer"]:
        assert legacy_marker not in vtxt, f"Legacy archive UI still present in vrm-viewer.mjs: found {legacy_marker}"

    # The file should reference the consolidated module by path or name
    assert '/js/archive-window.mjs' in vtxt or '/res/synth_webui/js/archive-window.mjs' in vtxt or 'archive-window.mjs' in vtxt, 'vrm-viewer.mjs should reference archive-window.mjs'
