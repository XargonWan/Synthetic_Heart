...
async def test_skip_whole_untagged_batches_then_process_next(monkeypatch):
    ...
    assert any("Skipping entire batch of" in m for m in logged), (
        f"Expected skip log in: {logged}"
    )
    assert any(
        "Dry-run clustering results" in m
        or "Processed clusters for current window" in m
        for m in logged
    ), f"Expected processing log in: {logged}"
