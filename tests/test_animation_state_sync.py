...
async def test_post_animation_state_calls_play_animation(monkeypatch):
    ...
    async def fake_play_animation(self, state, session_id, loop=True, context_id=None, priority=None, source=None):
        called['state'] = state
    ...
    class DummyReq:
        async def json(self):
            return {"state": "think", "session_id": "s1", "loop": False, "context_id": "ctx", "source": "test"}
...
async def test_get_animation_state_route_returns_payload():
    ...
    webui.animation_handler = type('H', (), {'get_current_animation_state': staticmethod(fake_current_state)})()
