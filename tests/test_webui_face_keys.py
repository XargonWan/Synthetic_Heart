def test_webui_face_keys_include_common_morphs():
    ...
    assert "sad" not in extra_block.lower(), (
        "Emotion 'sad' should not be present in facial morph aliases (extra list)"
    )
    ...
    for composite in ["valence", "arousal", "stress", "calm"]:
        assert composite not in extra_block.lower(), (
            f"Composite metric '{composite}' should not be present in the extra face aliases"
        )

    # Ensure exclusion is case-insensitive and persona emotion list is consulted
    assert "personaEmotionKeysLower" in html, (
        "Persona emotion exclusion should be present in getFaceKeys()"
    )

    # Ensure the client will consult persona emotion presets when resolving emotion keys (Feelings->face)
    assert "window.__synth_emotion_face_presets" in html, (
        "WebUI should reference persona emotion presets for mapping feelings to blendshapes"
    )
