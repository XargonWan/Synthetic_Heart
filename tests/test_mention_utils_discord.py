...
def make_reply_message(
    reply_author_id=None, reply_author_name=None, chat_type="group", text="hi"
):
...
async def test_is_message_for_bot_recognizes_reply_to_discord_bot():
    ...
    msg = make_reply_message(
        reply_author_id=555, reply_author_name="synth", chat_type="group", text="Thanks"
    )
    directed, reason = await is_message_for_bot(
        msg, bot, bot_username=None, human_count=2
    )
