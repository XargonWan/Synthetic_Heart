def test_raycast_global_is_window_scoped():
    ...
    for p in files:
        ...
        assert "window.__synthRaycastTargets" in txt, (
            f"window.__synthRaycastTargets not declared in {p}"
        )
        ...
        assert not m, (
            f"Undeclared assignment to __synthRaycastTargets found in {p}: {m.group(0) if m else ''}"
        )
