def test_debug_minimize_tool_present_in_vrm_viewer():
    ...
    assert "attachHeaderTools('debug', winbox, [" not in txt, "Legacy attachHeaderTools for debug still found in vrm-viewer.mjs"
    assert "/js/debug-window.mjs" in txt or "/res/synth_webui/js/debug-window.mjs" in txt or "debug-window.mjs" in txt, "vrm-viewer.mjs should reference debug-window.mjs loader"
...
def test_debug_minimize_tool_present_in_index_template():
    ...
    txt = p.read_text(encoding='utf-8')
    assert "/js/debug-window.mjs" in txt or "/res/synth_webui/js/debug-window.mjs" in txt or 'debug-window.mjs' in txt, "Index should load debug-window module"
    ...
    assert "title: 'Minimize'" in qtxt or 'title: "Minimize"' in qtxt, "Minimize tool not present in debug-window.mjs header tools"
