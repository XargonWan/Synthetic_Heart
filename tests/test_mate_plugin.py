...
from core.animation_uploads import sanitize_upload_id, record_upload, get_state_dir, ensure_uploads_root, delete_upload
from core.webui import SynthWebUIInterface, synth_webui_interface
...
async def test_send_mate_message_enqueues_outbox(monkeypatch, tmp_path):
    ...
    # Ensure outbox empty
    res = await webui.get_integration_outbox(type('R', (), {'query_params': {'source': 'mate'}})())
    assert json.loads(res.body)['messages'] == []

    action = {"type": "send_mate_message", "payload": {"text": "hello mate", "target": "unit123"}}
    await plugin.execute_action(action, context={}, bot=None, original_message=None)

    # Retrieve mate outbox
    res2 = await webui.get_integration_outbox(type('R', (), {'query_params': {'source': 'mate'}})())
...
async def test_promote_upload_requires_guard(monkeypatch, tmp_path):
    ...
    with pytest.raises(PermissionError):
        await plugin.execute_action({"type": "promote_upload", "payload": {"upload_id": upload_id, "target_skin": "TestSkin"}}, context={}, bot=None, original_message=None)
