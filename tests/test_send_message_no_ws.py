...
async def test_send_message_persists_when_no_websocket():
    ...
    with patch('core.chat_context_manager.save_response_message', AsyncMock()) as mock_save, \
         patch('core.webui.get_conn_ctx', AsyncMock()):
        # Call send_message with interface_path
        await webui.send_message({'interface_path': f'synth_webui/{session_id}', 'text': text})
