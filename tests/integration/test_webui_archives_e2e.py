...
@pytest.mark.asyncio
@pytest.mark.skipif(not RUN_INT, reason="Integration tests disabled (set RUN_INTEGRATION=1)")
async def test_webui_archive_restore_e2e():
    ...
    async with connect(ws_url) as ws:
        ...
        async with httpx.AsyncClient() as client:
            res = await client.post(f"{api_url}/api/chat/archive", json={"session_id": session_id})
            ...
            assert res.status_code == 200
            archives = res.json().get('archives', []) if res.json().get('success') else []
