def test_template_contains_cortex_controls():
    path = "core/webui_templates/synth_webui_index.html"
    with open(path, "r", encoding="utf-8") as f:
        content = f.read()
    assert 'id="cortex-engine-select"' in content
    assert 'id="cortex-login-btn"' in content
    assert 'id="cortex-engine-model"' in content
    assert 'id="cortex-engine-login-state"' in content
    ...
    assert "Select Cortex engine" not in content
    ...
    assert (
        "tryOpen" in content and "preferredProto" in content and "otherPort" in content
    )
