...
def test_nudge_present(monkeypatch):
    ...
    assert 'consider returning' in instr.lower() or 'consider returning a concise user-facing' in instr
...
async def test_preflight_telemetry_logging(monkeypatch):
    ...
    assert any('[json_prompt][preflight_summary]' in str(m) and 'strategy=free_db' in str(m) for m in captured)
