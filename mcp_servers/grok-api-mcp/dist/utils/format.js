 ...
 */
export function parseJSON(rawContent, filename) {
    try {
...
export function formatMarkdownTable(headers, rows) {
    ...
    const dataRows = rows.map((row) => {
        const normalizedRow = row.length === headerCount
            ? row
 ...
 */
export function formatModelValue(modelValue, options) {
    const wrapModel = options?.markdown ? (m) => `\`${m}\`` : (m) => m;
