 ...
 */
function withErrorHandler(toolName, handler) {
...
const TOOLS = {
    SEARCH_DOCS: {
        name: "search_docs",
        description: "Search bundled Grok API documentation for relevant information. Use this for quick lookups of API features, models, function calling, and built-in tools.",
    },
    GET_DOC_PAGE: {
        name: "get_doc_page",
        description: "Fetch fresh documentation from docs.x.ai. Use this when you need the latest information or content not available in bundled docs.",
    },
    LIST_ENDPOINTS: {
        name: "list_api_endpoints",
        description: "List available xAI API endpoints. Filter by category: chat, images, videos, voice, models, files, batch, collections, api-keys, billing, team, audit.",
    },
    LIST_MODELS: {
        name: "list_models",
...
// Tool: search_docs
server.tool(TOOLS.SEARCH_DOCS.name, TOOLS.SEARCH_DOCS.description, {
    ...
    return withErrorHandler(TOOLS.SEARCH_DOCS.name, () => {
        ...
        if (results.length === 0) {
            return formatMcpResponse(`No results found for "${query}". Try different keywords or use ${TOOLS.GET_DOC_PAGE.name} to fetch specific documentation pages.`);
        }
...
// Tool: get_doc_page
server.tool(TOOLS.GET_DOC_PAGE.name, TOOLS.GET_DOC_PAGE.description, {
    path: z
        .string()
    ...
    try {
        ...
        return formatMcpResponse(`# Documentation: ${path}\n\nSource: ${DOCS_BASE_URL}/${path}\n\n${content}`);
    }
    catch (error) {
        return formatMcpResponse(`Error fetching documentation: ${getErrorMessage(error)}\n\nTry using ${TOOLS.SEARCH_DOCS.name} to find bundled documentation instead.`, true);
    }
});
// Tool: list_api_endpoints
server.tool(TOOLS.LIST_ENDPOINTS.name, TOOLS.LIST_ENDPOINTS.description, {
    category: z
        .string()
        .optional()
...
// Tool: list_models
server.tool(TOOLS.LIST_MODELS.name, TOOLS.LIST_MODELS.description, {}, async () => {
