...
class ImageProcessor:
    ...
    def __init__(self):
        log_info(
            f"[image_processor] Initialized with RESTRICT_ACTIONS={str(RESTRICT_ACTIONS).lower()}"
        )
    ...
    def _check_access_permissions(
        self, user_id: Union[int, str], interface_name: str, has_trigger: bool
    ) -> Tuple[bool, str]:
        """
        ...
        if restrict_mode == "deny_all":
            return (
                False,
                "Image processing is completely disabled (RESTRICT_ACTIONS=deny_all)",
            )
        ...
        if restrict_mode == "on":
            return (
                False,
                "Image processing is completely disabled (RESTRICT_ACTIONS=on)",
            )
        ...
        if restrict_mode == "trainer_only":
            ...
            if not is_trainer:
                return (
                    False,
                    f"Image processing restricted to trainers only (user {user_id} is not trainer for {interface_name})",
                )

            return (
                True,
                f"Access granted to trainer {user_id} for interface {interface_name}",
            )

        return False, f"Unknown RESTRICT_ACTIONS mode: {restrict_mode}"

    async def should_process_image(
        self, context: AbstractContext, has_trigger: bool = False
    ) -> Tuple[bool, str]:
        """
        ...
        allowed, reason = self._check_access_permissions(
            user_id, interface_name, has_trigger
        )

        log_debug(
            f"[image_processor] Access check for user {user_id} on {interface_name}: {allowed} - {reason}"
        )

        return allowed, reason

    async def prepare_image_for_llm(
        self, image_data: Dict[str, Any], context: AbstractContext
        ...
        try:
            ...
            llm_image_data = {
                ...
                "metadata": {
                    "timestamp": getattr(context.message, "timestamp", None)
                    if context.message
                    else None,
            ...
            log_info(
                f"[image_processor] Prepared image for LLM: {llm_image_data['source']}"
            )
    ...
    async def forward_to_llm(self, processed_image_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            log_info(
                f"[image_processor] Forwarding image to LLM from user {source_info.get('user_id')}"
            )
...
async def process_image_message(
    ...
    return await processor.process_image_message(
        image_data, context, has_trigger, forward_to_llm
    )


__all__ = ["ImageProcessor", "get_image_processor", "process_image_message"]
