...
def ensure_message_user_fields(message: Any) -> None:
    ...
    if user is None:
        ...
        message.from_user = SimpleNamespace(id=0, username=None, first_name=None, full_name=None)
        return
    ...
    if getattr(user, 'full_name', None) is None:
        candidate = getattr(user, 'first_name', None) or getattr(user, 'username', None) or getattr(user, 'id', None)
