...
class SynthWebUIInterface:
    ...
    def __init__(self, autostart: bool = True) -> None:
        ...
        log_info(
            f"{LOG_PREFIX} Animation handler initialized (single websocket sender)",
            log_file=WEBUI_LOG,
        )
    ...
    def _render_index(self) -> str:
        ...
        try:
            ...
            template_path = (
                Path(__file__).parent / "webui_templates" / "synth_webui_index.html"
            )
...
"""
    ...
    async def websocket_endpoint(self, websocket: WebSocket):
        ...
        try:
            if self.animation_handler:
                current_anim_state = (
                    self.animation_handler.get_current_animation_state()
                )
                if current_anim_state["animation_file"]:
                    ...
                    message = {
                        ...
                        "loop": current_anim_state["descriptor"].get("play_once", False)
                        is False
                        if current_anim_state["descriptor"]
                        else True,
                        "descriptor": current_anim_state["descriptor"],
                    }
                    await websocket.send_json(message)
                    log_debug(
                        f"{LOG_PREFIX} Sent current animation state to new session {session_id}: {current_anim_state['state']}"
                    )
            ...
            log_warning(
                f"{LOG_PREFIX} Failed to send animation state to new session {session_id}: {anim_exc}"
            )
        ...
        try:
            if self.persona_manager:
                ...
                log_debug(
                    f"{LOG_PREFIX} Set initial idle animation for session {session_id}"
                )
            else:
                log_debug(
                    f"{LOG_PREFIX} Persona manager not available, skipping initial animation for session {session_id}"
                )
        except Exception as anim_exc:
            log_warning(
                f"{LOG_PREFIX} Failed to set initial idle animation for session {session_id}: {anim_exc}"
            )
    ...
    async def _broadcast_animation_state(
        ...
        """
        log_debug(
            f"{LOG_PREFIX} [_broadcast_animation_state] CALLED: state={state}, animation={animation_file}, has_descriptor={descriptor is not None}"
        )
        ...
        log_info(
            f"{LOG_PREFIX} Broadcasting animation state to {client_count} clients: {state.value}/{animation_file}",
            log_file=WEBUI_LOG,
        )
        ...
        for session_id, websocket in self.connections.items():
            try:
                await websocket.send_json(message)
                log_debug(
                    f"{LOG_PREFIX} ✓ Sent animation state to session {session_id}: {state.value}"
                )
            except Exception as exc:
                log_warning(
                    f"{LOG_PREFIX} Failed to broadcast animation state to session {session_id}: {exc}"
                )
        ...
        try:
            ...
            try:
                ...
                if (
                    ...
                    and current.get("animation_state")
                ):
            ...
            for sid, websocket in list(self.connections.items()):
                try:
                    ...
                    log_warning(
                        f"{LOG_PREFIX} Failed to send animation_state to {sid}: {exc}"
                    )
        except Exception as exc:
            log_warning(
                f"{LOG_PREFIX} _broadcast_animation_state_summary failed: {exc}"
            )
    ...
    async def send_tts_audio(
        ...
        """
        Send TTS audio to a WebUI session for lip-sync playback.

        Args:
            session_id: The WebUI session ID (extracted from interface_path).
            audio_path: Path to the audio file (will be converted to URL).
            text: Optional text to display as a message before playing audio.

        Returns:
            True if audio was dispatched, False otherwise.
        """
        # Handle interface_path format: extract session_id from "synth_webui/session_id"
        if "/" in str(session_id):
            ...
            if len(parts) >= 2 and parts[0] == INTERFACE_NAME:
                session_id = parts[1]
                log_debug(
                    f"{LOG_PREFIX} Extracted session {session_id} from interface_path"
                )

        websocket = self.connections.get(session_id)
        if not websocket:
            log_warning(
                f"{LOG_PREFIX} send_tts_audio: no active websocket for session {session_id}. "
                f"Active sessions: {list(self.connections.keys())}"
            )
            return False
        ...
        if text:
            ...
            await self._append_history(session_id, "synth", text)
            log_info(
                f"{LOG_PREFIX} Sent TTS text to session {session_id}: "
                f"{text[:80]}{'...' if len(text) > 80 else ''}"
            )

        # Convert audio path to URL
        # Audio files are stored in res/synth_webui/static/audio/tts/
        # They are served at /static/audio/tts/<filename>
        from pathlib import Path

        audio_file = Path(audio_path)
        if audio_file.exists():
            # Build relative URL from filename
            audio_url = f"/static/audio/tts/{audio_file.name}"
            log_info(
                f"{LOG_PREFIX} Sending TTS audio to session {session_id}: {audio_url}"
            )
            ...
            log_warning(f"{LOG_PREFIX} TTS audio file not found: {audio_path}")
            return False
    ...
    async def config_summary(self):
        ...
        for entry in definitions:
            ...
            if (
                "llm_engine" in entry.get("tags", [])
                and entry.get("component") not in loaded_llm_engines
            ):
                continue
            ...
            if self.persona_manager:
                for session in list(self.connections.keys()):
                    try:
                        ...
                        await self.persona_manager.set_animation_state(
                            "idle", session_id=session
                        )
                        log_debug(
                            f"{LOG_PREFIX} Preloaded idle animation for session {session}"
                        )
                    except Exception as anim_exc:
                        log_warning(
                            f"{LOG_PREFIX} Failed to preload idle for session {session}: {anim_exc}"
                        )
            else:
                log_debug(
                    f"{LOG_PREFIX} Persona manager not available - skipping idle preload for connected clients"
                )
        except Exception as exc:
        ...
        try:
            ...
            if destination.exists():
                try:
                    ...
                    log_error(
                        f"{LOG_PREFIX} Failed to cleanup partial upload: {cleanup_exc}"
                    )
    ...
    async def set_llm_engine(self, request: Request):
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail="Unable to access LLM configuration"
            ) from exc
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to activate LLM '{name}'"
            ) from exc
    ...
    async def llm_login(self, request: Request):
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail="Unable to access LLM registry"
            ) from exc

        if not engine:
            raise HTTPException(
                status_code=404, detail=f"LLM engine '{name}' not loaded"
            )
        ...
        try:
            ...
            if not isinstance(engine, SeleniumLLMBase):
                raise HTTPException(
                    status_code=400, detail=f"LLM engine '{name}' is not Selenium-based"
                )
        ...
        try:
            ...
            try:
                ...
                log_info(
                    f"{LOG_PREFIX} Started login flow for LLM '{name}' (task: {task})"
                )
            except RuntimeError:
    ...
    async def reload_component(self, request: Request):
        ...
        if not component_type or component_type not in ["interface", "plugin", "llm"]:
            raise HTTPException(
                status_code=400,
                detail="Missing or invalid 'type'. Must be 'interface', 'plugin', or 'llm'",
            )
        ...
        try:
            if component_type == "interface":
                ...
                if component_name not in llm_registry.get_available_engines():
                    raise HTTPException(
                        status_code=404,
                        detail=f"LLM engine '{component_name}' not found",
                    )
                ...
                try:
                    ...
                    log_info(
                        f"{LOG_PREFIX} LLM engine '{component_name}' reloaded successfully"
                    )
                    return JSONResponse(
                        {
                            "status": "ok",
                            "message": f"LLM engine '{component_name}' reloaded successfully",
                        }
                    )
                except Exception as load_exc:
                    log_error(
                        f"{LOG_PREFIX} Failed to reload LLM engine '{component_name}': {load_exc}"
                    )
                    raise HTTPException(
                        status_code=500,
                        detail=f"Failed to reload LLM engine '{component_name}': {str(load_exc)}",
                    ) from load_exc
    ...
    def _ensure_tls_files(self) -> None:
        ...
        try:
            log_info(
                f"{LOG_PREFIX} Generating self-signed certificate using openssl at {default_cert}"
            )
