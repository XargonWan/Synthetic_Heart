...
"""
    ...
    try:
        ...
        if (
            not correction_message.interface_path
            and context
            and "interface_path" in context
        ):
        ...
        if corrected_json_str:
            log_info(
                f"[action_parser] Received corrected JSON: {corrected_json_str[:200]}..."
            )
            ...
            if actions_to_run:
                log_info(
                    f"[action_parser] Executing {len(actions_to_run)} corrected actions"
                )
