...
def validate_action(
    ...
    if not action_type:
        ...
        # Check for Grillo internal beat mistake - message_grillo is not valid
        if action_type == "message_grillo":
            errors.append(
                "Invalid action 'message_grillo' - Grillo is an internal beat system, not an interface. "
                "For internal beats, use only 'create_personal_diary_entry' (no message actions). "
                "For outreach beats, use 'message_telegram_bot' or 'message_discord_bot'."
            )
        else:
            ...
            if hasattr(plugin, "get_supported_action_types"):
                ...
                if action_types and action_type in action_types:
                    ...
                    if action_type.startswith("message_"):
                        log_debug(
                            f"[action_parser] ⚡ Optimization: Stopping search for message action {action_type} after first match"
                        )
                        break
                    continue
...
async def run_action(action: Any, context: Dict[str, Any], bot, original_message):
    ...
    try:
        if isinstance(context, dict) and bool(context.get("preflight")):
            PRELIGHT_BLOCKED_ACTIONS = {
                "create_personal_diary_entry",
                "create_diary_entry",
            }
            if action_type in PRELIGHT_BLOCKED_ACTIONS:
                log_info(
                    f"[action_parser] ⚠️ Skipping action '{action_type}' during preflight to avoid side-effects"
                )
...
async def run_actions(actions: Any, context: Dict[str, Any], bot, original_message):
    ...
    for idx, action in enumerate(actions):
        try:
            ...
            # Prevent re-calling the same action when processing action results
            # This stops infinite loops where LLM calls memory_search again after receiving results
            source_action_type = (
                context.get("system_message", {}).get("action_type")
                if isinstance(context.get("system_message"), dict)
                else None
            )
            is_action_result = (
                context.get("system_message", {}).get(
                    "is_action_result_delivery", False
                )
                if isinstance(context.get("system_message"), dict)
                else False
            )
            if is_action_result and action_type == source_action_type:
                error_msg = f"Blocked re-execution of '{action_type}' - already received results from this action"
            ...
            if is_from_llm:
                try:
                    ...
                    synth_mode = str(
                        config_registry.get_value("SYNTH_AUTONOMY_MODE", "suggest")
                        or "suggest"
                    ).lower()
                    llm_unsafe_override = bool(
                        config_registry.get_value(
                            "LLM_AUTO_EXECUTE_UNSAFE_ACTIONS", False
                        )
                    )
                    allowed_autonomy = (
                    ...
                    # Mark as unfixable=True so corrector doesn't try to "fix" a policy restriction
...
async def gather_static_injections(message=None, context_memory=None) -> dict:
    ...
    injections: dict = {}
    log_debug("[action_parser] 🔍 gather_static_injections() CALLED")
    try:
        plugins_list = _load_action_plugins()
        log_debug(
            f"[action_parser] Found {len(list(plugins_list))} plugins to check for static_inject"
        )
        ...
        for plugin in plugins_list:
            try:
                ...
                log_debug(
                    f"[action_parser] Plugin {plugin.__class__.__name__}: supported={supported}, has_method={has_method}"
                )
                ...
                log_debug(
                    f"[action_parser] 🎯 Calling get_static_injection() on {plugin.__class__.__name__}"
                )
                ...
                try:
                    ...
                    try:
                        ...
                        log_error(
                            f"[action_parser] Error calling get_static_injection() on {plugin.__class__.__name__}: {inner_e}"
                        )
                        continue

                if inspect.iscoroutine(result):
                    try:
                        # Add timeout to prevent slow plugins from blocking the entire chain
                        result = await asyncio.wait_for(result, timeout=5.0)
                    except asyncio.TimeoutError:
                        log_warning(
                            f"[action_parser] Timeout waiting for static injection from {plugin.__class__.__name__}"
                        )
                        continue
                    except Exception as await_e:
                        log_error(
                            f"[action_parser] Error awaiting static injection from {plugin.__class__.__name__}: {await_e}"
                        )
                        continue

                log_debug(
                    f"[action_parser] ✅ Got result from {plugin.__class__.__name__}: {list(result.keys()) if isinstance(result, dict) else type(result)}"
                )
