...
async def generic_context_command(
...
async def generic_last_chats_command(
...
async def generic_help_command(
    abstract_context: AbstractContext, reply_fn: Optional[Callable] = None
):
...
async def generic_diary_command(
    abstract_context: AbstractContext,
    reply_fn: Optional[Callable] = None,
    args: str = "",
):
    ...
    try:
        ...
        if not entries:
            ...
            response = (
                f"📔 **synth's Diary - Last {days} days ({len(entries)} entries)**\n\n"
            )
...
def create_command_wrapper(
    interface_name: str, adapter_fn: Callable, original_params_fn: Callable
):
    ...
    def command_wrapper(generic_command_fn):
        async def wrapper(*args, **kwargs):
            try:
                ...
                log_warning(
                    f"[generic_commands] Error in {interface_name} command wrapper: {e}"
                )
