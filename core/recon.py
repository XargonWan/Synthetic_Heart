...
async def gather_recon_contributions(message=None, context_memory=None, text: str | None = None, tags: List[str] | None = None, keywords: List[str] | None = None, max_results: int = 5) -> List[Dict[str, Any]]:
    ...
    for plugin in plugins:
        try:
            if hasattr(plugin, "get_recon_contributions"):
                ...
                try:
                    result = fn(message=message, context_memory=context_memory, text=text, tags=tags, keywords=keywords, max_results=max_results)
                except TypeError:
