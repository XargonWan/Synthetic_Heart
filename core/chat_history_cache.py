    ...
    try:
        ...
        async with get_conn_ctx() as conn:
            async with conn.cursor() as cur:
                ...
                # live-sync path itself (to avoid echo loops) or the message
                # was generated by the bot (sender "self") — forwarding bot
                # responses into Live would cause the voice model to treat
                # Synth's own Telegram/text replies as user speech and react
                # to them, creating a feedback loop. We also filter out
                # background periodic agents like grillo and diary to avoid
                # interrupting the voice session with internal bot thoughts.
                if (
