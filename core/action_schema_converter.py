...
def normalize_action_schema(action_name: str, action_def: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "schema" in action_def and "brief" in action_def:
        log_debug(f"[action_schema_converter] Action '{action_name}' already in new format")
        return action_def
    
    # Convert from old format
    if "description" in action_def or "required_fields" in action_def:
        log_debug(f"[action_schema_converter] Converting action '{action_name}' from old format to new")
        ...
        examples = action_def.get("examples", []) if isinstance(action_def.get("examples"), list) else []
    ...
    log_debug(f"[action_schema_converter] Action '{action_name}' format unrecognized, passing through")
    return action_def


def extract_for_llm_prompt(action_name: str, action_def: Dict[str, Any]) -> Dict[str, Any]:
...
def extract_for_corrector(action_name: str, action_def: Dict[str, Any]) -> Dict[str, Any]:
