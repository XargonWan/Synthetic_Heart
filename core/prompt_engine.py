...
async def llm_memory_search_preflight(
    *,
    ...
    try:
        ...
        log_debug(
            f"[json_prompt] LLM preflight: unable to load active engine ({active_llm}): {e}"
        )
        return []
    ...
    try:
        ...
        full_actions = (core_initializer.actions_block or {}).get(
            "available_actions", {}
        )
    ...
    system_prompt = (
        ...
        f"Set payload.max_results to {int(max_results)}." + schema_hint
    )
    ...
    try:
        ...
        log_warning(
            f"[json_prompt] LLM preflight: engine.generate_response failed: {e}"
        )
        return []
    ...
    try:
        ...
        try:
            if isinstance(llm_text, str) and ("{" in llm_text or "[" in llm_text):
                log_info(
                    "[json_prompt] LLM preflight: attempting corrector middleware on malformed JSON output"
                )
                try:
                    ...
                    if corrected and isinstance(corrected, str):
                        try:
                            parsed = extract_json_from_text(
                                corrected, return_metadata=False
                            )
                            # replace llm_text with corrected for telemetry/logging
                            llm_text = corrected
                        except Exception as e2:
                            log_debug(
                                f"[json_prompt] LLM preflight: corrected text still invalid JSON: {e2}"
                            )
                except Exception as e1:
                    log_debug(
                        f"[json_prompt] LLM preflight: corrector middleware failed: {e1}"
                    )
        except Exception:
            pass
    ...
    if isinstance(parsed, dict) and isinstance(parsed.get("actions"), list):
        ...
        and parsed.get("type") == "memory_search"
    ...
    try:
        log_info(
            f"[json_prompt][preflight_summary] strategy=llm_action snippets={len(snippets)} delivered_to_llm={delivered}"
        )
    except Exception:
        pass
    ...
    try:
        ...
        if extra_actions:
            log_info(
                f"[json_prompt] LLM preflight returned extra actions (ignored): {[a.get('type') for a in extra_actions]}"
            )
    except Exception:
        pass
...
async def build_json_prompt(
    ...
    try:
        if bool(
            config_registry.get_value("MEMORY_SEARCH_PREFLIGHT", False, value_type=bool)
        ):
            try:
                preflight_max = int(
                    config_registry.get_value(
                        "MEMORY_SEARCH_PREFLIGHT_MAX_RESULTS", 5, value_type=int
                    )
                    or 5
                )
            except Exception:
            ...
            try:
                timeout = int(
                    config_registry.get_value(
                        "MEMORY_SEARCH_PREFLIGHT_TIMEOUT", 180, value_type=int
                    )
                    or 180
                )
            except Exception:
            ...
            if strategy == "llm_action":
                log_info(
                    f"[json_prompt] Preflight enabled: running LLM-driven memory_search preflight (max={preflight_max}) with timeout={timeout}s"
                )
                try:
                    ...
                    log_warning(
                        f"[json_prompt] LLM preflight timed out after {timeout}s; proceeding without preflight snippets"
                    )
                    preflight_results = []
                except Exception as e:
                    log_warning(
                        f"[json_prompt] LLM preflight failed: {e}; falling back to free_memory_search (max={preflight_max})"
                    )
                    try:
                        preflight_results = await asyncio.wait_for(
                            free_memory_search(text, limit=preflight_max),
                            timeout=timeout,
                        )
                    except asyncio.TimeoutError:
                        log_warning(
                            f"[json_prompt] free_memory_search fallback timed out after {timeout}s; proceeding without preflight snippets"
                        )
                        preflight_results = []
                    except Exception as e2:
                        log_warning(
                            f"[json_prompt] free_memory_search fallback failed: {e2}; proceeding without preflight snippets"
                        )
                        preflight_results = []
            else:
                log_info(
                    f"[json_prompt] Preflight enabled: running free_memory_search with max={preflight_max} (strategy={strategy}) with timeout={timeout}s"
                )
                try:
                    preflight_results = await asyncio.wait_for(
                        free_memory_search(text, limit=preflight_max), timeout=timeout
                    )
                except asyncio.TimeoutError:
                    log_warning(
                        f"[json_prompt] free_memory_search timed out after {timeout}s; proceeding without preflight snippets"
                    )
                    preflight_results = []
                except Exception as e:
                    log_warning(
                        f"[json_prompt] free_memory_search failed: {e}; proceeding without preflight snippets"
                    )
                    preflight_results = []
            ...
            if preflight_results:
                ...
                log_info(
                    f"[json_prompt] ⏱️ Added {added} preflight snippets in {time.time() - start_time:.2f}s"
                )
                try:
    ...
    try:
        if bool(
            ...
            json_instructions = (
                f"{json_instructions} {memory_search_instr} {gentle_nudge}"
            )
            json_instructions = " ".join((json_instructions or "").split())
    except Exception as e:
        log_debug(
            f"[prompt_engine] Could not add memory_search instruction to build_json_prompt: {e}"
        )
