...
class ReconAgentIntentPlugin:
    ...
    def get_recon_instruction(self) -> str:
        return (
            ...
            "replying with knowledge or conversation. Decide by answering these "
            "questions about the request: (1) is it articulated enough that it "
            "must be carried out in several steps? (2) does it require reading "
            "or modifying files, inspecting the codebase, or running commands? "
            "(3) does it require producing substantial content or an "
            "operational result, rather than a simple conversational reach-out? "
            "If the answer to ANY of these is yes, agentic work is needed. It is "
            "NOT needed for greetings, small talk, opinions, or explanations "
            "from general knowledge that a single direct reply fully satisfies. "
