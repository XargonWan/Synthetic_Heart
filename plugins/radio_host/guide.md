...
| `AZURACAST_BASE_URL` | string | Your AzuraCast instance URL, e.g. `https://radio.example.com`. |
| `AZURACAST_API_KEY` | password (masked) | API key with station management permissions. |
| `AZURACAST_STATION_ID` | string | Station shortcode from AzuraCast, visible in the URL when viewing the station admin page. |
| `RADIO_HOST_LANGUAGE` | string | Language for DJ comments, e.g. `English`, `Italian`, `Spanish` (default: `English`). |
...
- `style` — Context hint for the comment type. Allowed values:
  `transition` (between songs), `intro`, `outro`, `news`, `shoutout`.
