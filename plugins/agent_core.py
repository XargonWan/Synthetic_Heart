...
class AgentCorePlugin(AIPluginBase):
    ...
    async def approve_action(
        ...
        *,
        ...
        await self._insert_action_exec(
            ...
            "shell_command",
            command or {},
            "processed" if exec_result else "pending",
            exec_result,
            None,
        )
    ...
    async def attach_to_active_engine(self, get_active_engine_name_fn) -> None:
        ...
        try:
            ...
            if engine is None:
                log_debug(
                    f"[agent_core] Engine {engine_name} not loaded; cannot attach"
                )
                return
            ...
            if attach_fn and callable(attach_fn):
                try:
                    attach_fn(self)
                    log_debug(
                        f"[agent_core] Attached to engine {engine_name} via attach_agent"
                    )
                    return
                except Exception:
                    log_debug(
                        f"[agent_core] Engine {engine_name} attach_agent failed, falling back to attribute set"
                    )
            # Fallback: set attribute on engine instance
            setattr(engine, "_agent_plugin", self)
            log_debug(
                f"[agent_core] Attached to engine {engine_name} via attribute set"
            )
        except Exception as e:
        ...
        try:
            if action_type == "approve_action":
                ...
                return await self.approve_action(
                    proposal_id=proposal_id, approver=approver, command=command
                )
        ...
        try:
            ...
            if detach_fn and callable(detach_fn):
                try:
                    detach_fn(self)
                    log_debug(
                        f"[agent_core] Detached from engine {engine_name} via detach_agent"
                    )
                    return
                except Exception:
                    log_debug(
                        f"[agent_core] Engine {engine_name} detach_agent failed, falling back to attribute delete"
                    )
            # Fallback
            try:
