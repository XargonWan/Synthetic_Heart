...
try:
    ...
    def _ask(prompt: str, default: str = "") -> str:
        val = Prompt.ask(f"  [cyan]{prompt}[/cyan] [dim]( < = back )[/dim]", default=default)
        if val.strip() == "<":
            raise _Back()
        return val
    ...
    def _choice(prompt: str, choices: list[str], default: str) -> str:
        ...
        val = Prompt.ask(
            f"  [cyan]{prompt}[/cyan] [{joined}] [dim]( < = back )[/dim]", default=default
        )
        if val.strip() == "<":
            raise _Back()
...
def _write_env(values: dict[str, str]) -> None:
    ...
    sections: list[tuple[str, list[str]]] = [
        ("Persona & engine defaults", ["SYNTH_NAME", "SYNTH_PROFILE", "BASE_CORTEX", "ACTIVE_VOX_ENGINE", "ACTIVE_AURIS_ENGINE", "SYNTH_AUTONOMY_MODE"]),
        ("LLM credentials", ["GEMINI_API_KEY", "OPENAI_API_KEY", "OPENROUTER_API_KEY", "ANTHROPIC_API_KEY"]),
        ("Interface credentials", ["BOTFATHER_TOKEN", "DISCORD_BOT_TOKEN", "MATRIX_HOMESERVER", "MATRIX_USER", "MATRIX_PASSWORD"]),
        ("Trainer / notifications", ["TRAINER_CHAT_ID", "LOG_CHAT_ID", "LOG_CHAT_INTERFACE"]),
        ("WebUI", ["SYNTH_WEBUI_HTTP_PORT", "SYNTH_WEBUI_HTTPS_PORT", "SYNTH_WEBUI_TLS", "TZ"]),
        ("Database", ["DB_HOST", "DB_PORT", "DB_USER", "DB_PASS", "DB_NAME"]),
        ("SOUL / Embedder", ["SOUL_REPOSITORY_BACKEND", "SOUL_POSTGRES_DSN", "SOUL_EMBEDDER_ID", "SOUL_EMBEDDER_USE_GPU"]),
        ("Behavior defaults", ["PROJECT_DEFAULT_LANGUAGE", "PROJECT_DEFAULT_TONE", "DIARY_HISTORY_DAYS", "GRILLO_BEAT_INTERVAL", "ENABLE_RECON", "ENABLE_DEBRIEF", "SYNTH_HOST_OS"]),
        ("Observability", ["LANGFUSE_ENABLED", "LANGFUSE_HOST", "LANGFUSE_PUBLIC_KEY", "LANGFUSE_SECRET_KEY", "CORTEX_API_LOG_ENABLED"]),
    ]
    written: set[str] = set()
    for section_name, keys in sections:
        section_lines = [f"{k}={values[k]}" for k in keys if k in values]
        if section_lines:
            lines += [f"# {'─' * 60}", f"# {section_name}", f"# {'─' * 60}"] + section_lines + [""]
            written.update(keys)
...
def stage_check(_args: argparse.Namespace) -> bool:
    ...
    if uv_path:
        ...
        rc = _run_visible(["powershell", "-NoProfile", "-Command", "irm https://astral.sh/uv/install.ps1 | iex"])
        if rc != 0:
    ...
    if node_path:
        ...
        _warn("Node.js not found — GitNexus MCP server won't work. Install via: winget install OpenJS.NodeJS.LTS")
...
def stage_db(args: argparse.Namespace) -> tuple[bool, dict[str, str]]:
    ...
    _hint("SyntH uses MariaDB as its primary database for chat history, memory, config, and emotion state.")
    db_conf: dict[str, str] = {"DB_HOST": "127.0.0.1", "DB_PORT": "3306", "DB_USER": "synth", "DB_NAME": "synth"}

    _print("  Checking MariaDB service status...")
    for svc_name in ("MariaDB", "MySQL"):
        result = subprocess.run(["sc", "query", svc_name], capture_output=True, text=True)
        ...
        _warn("MariaDB service not found. If you installed via the SyntH installer, this is unexpected.")
        _warn("Re-run: python scripts/windows_setup.py --stage db  after starting the service.")

    if args.non_interactive:
        root_pass = ""
        db_pass = _generate_password()
    else:
        _hint("Enter the MariaDB root password. For a fresh install from the SyntH installer, this is blank — just press Enter.")
        root_pass = _ask("MariaDB root password", default="")
        _hint("This password will be set for the 'synth' database user. Leave blank to auto-generate a secure one.")
        db_pass = _ask("Password for 'synth' DB user (blank = auto-generate)", default="")
        if not db_pass:
            db_pass = _generate_password()
            _print(f"  Generated password: [bold magenta]{db_pass}[/bold magenta]" if HAS_RICH else f"  Generated password: {db_pass}")
    db_conf["DB_PASS"] = db_pass

    try:
        import pymysql
        conn = pymysql.connect(host="127.0.0.1", port=3306, user="root", password=root_pass, connect_timeout=10)
        with conn.cursor() as cur:
            cur.execute("CREATE DATABASE IF NOT EXISTS `synth` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci")
            cur.execute("CREATE USER IF NOT EXISTS 'synth'@'localhost' IDENTIFIED BY %s", (db_pass,))
    ...
    if INIT_SQL.exists():
        try:
            conn = pymysql.connect(host="127.0.0.1", port=3306, user="synth", password=db_pass, database="synth", connect_timeout=10)
...
def stage_env(args: argparse.Namespace, db_conf: dict[str, str]) -> bool:
    ...
    if ENV_FILE.exists() and not args.non_interactive:
        if not _confirm(".env already exists — overwrite with new values?", default=False):
    ...
    group_a = _run_steps([
        _Step("SYNTH_NAME", "AI name", "Synth",
              "The name your AI responds to and uses in conversations."),
        _Step("SYNTH_PROFILE", "Persona profile", "You are Synth, a modular AI persona.",
              "System prompt prefix defining personality. Keep it concise — it's prepended to every request."),
        _Step("BASE_CORTEX", "Base LLM engine", existing.get("BASE_CORTEX", "gemini"),
        ...
        _Step("ACTIVE_VOX_ENGINE", "TTS engine", existing.get("ACTIVE_VOX_ENGINE", "edge_tts"),
        ...
        _Step("ACTIVE_AURIS_ENGINE", "STT engine", existing.get("ACTIVE_AURIS_ENGINE", "vosk"),
              "Speech-to-text engine. vosk = offline (~50 MB model). whisper = OpenAI Whisper, better accuracy.",
              choices=["vosk", "whisper"]),
        _Step("SYNTH_AUTONOMY_MODE", "Autonomy mode", existing.get("SYNTH_AUTONOMY_MODE", "always_ask"),
    ...
    ], existing, args.non_interactive)
    values.update(group_a)

    # ── Group B: LLM credentials ──────────────────────────────────────────────
    _section("B. LLM API keys  [dim](leave blank to skip)[/dim]" if HAS_RICH else "B. LLM API keys (leave blank to skip)")
    _hint("Only fill in keys for engines you plan to use. All can be added later by re-running --reconfigure.")
    b_steps = [
        _Step("GEMINI_API_KEY", "Gemini API key", existing.get("GEMINI_API_KEY", ""),
              "Required for BASE_CORTEX=gemini. Get one at https://aistudio.google.com/apikey"),
        _Step("OPENAI_API_KEY", "OpenAI API key", existing.get("OPENAI_API_KEY", ""),
              "Required for BASE_CORTEX=openapi or any OpenAI-compatible endpoint."),
        _Step("OPENROUTER_API_KEY", "OpenRouter API key", existing.get("OPENROUTER_API_KEY", ""),
              "Required for BASE_CORTEX=openrouter. Gives access to 100+ models via one key."),
        _Step("ANTHROPIC_API_KEY", "Anthropic API key", existing.get("ANTHROPIC_API_KEY", ""),
    ...
    _hint("Chat interfaces let users talk to SyntH via messaging platforms. All optional.")
    if not args.non_interactive:
        _hint("Telegram: fast setup, great for mobile. Get a token from @BotFather.")
        if _confirm("Enable Telegram bot?", default=bool(existing.get("BOTFATHER_TOKEN"))):
            try:
        ...
        _hint("Discord: rich embeds, server-based. Create a bot at discord.com/developers.")
        if _confirm("Enable Discord bot?", default=bool(existing.get("DISCORD_BOT_TOKEN"))):
            try:
                t = _ask("Discord bot token", default=existing.get("DISCORD_BOT_TOKEN", ""))
                if t:
    ...
    _hint("Optional: the 'trainer' user receives status messages and can send commands to SyntH.")
    d_steps = [
        _Step("TRAINER_CHAT_ID", "Trainer chat ID", existing.get("TRAINER_CHAT_ID", ""),
              "Format: interface/user_id — e.g. telegram_bot/123456789. Leave blank to skip."),
        _Step("LOG_CHAT_ID", "Log chat ID", existing.get("LOG_CHAT_ID", ""),
              "Where to send system log messages. Same format as trainer ID."),
        _Step("LOG_CHAT_INTERFACE", "Log chat interface", existing.get("LOG_CHAT_INTERFACE", ""),
    ...
    # ── Group E: WebUI ────────────────────────────────────────────────────────
    _section("E. WebUI")
    _hint("SyntH's browser-based control panel. Accessible at http://localhost:<port> by default.")
    group_e = _run_steps([
        _Step("SYNTH_WEBUI_HTTP_PORT", "HTTP port", existing.get("SYNTH_WEBUI_HTTP_PORT", "8001"),
              "Default 8001. Change if that port is already in use on your machine."),
        _Step("SYNTH_WEBUI_HTTPS_PORT", "HTTPS port", existing.get("SYNTH_WEBUI_HTTPS_PORT", "8000"),
              "Only used if TLS is enabled below."),
        _Step("SYNTH_WEBUI_TLS", "Enable TLS (1/0)", existing.get("SYNTH_WEBUI_TLS", "0"),
              "Set to 1 if you have SSL certificates and want HTTPS. Leave 0 for local use."),
        _Step("TZ", "Timezone", existing.get("TZ", "UTC"),
              "IANA timezone name, e.g. Europe/London, America/New_York. Affects log timestamps."),
    ], existing, args.non_interactive)
    values.update({k: v or s.default for s, (k, v) in zip(
        [_Step("SYNTH_WEBUI_HTTP_PORT","","8001"), _Step("SYNTH_WEBUI_HTTPS_PORT","","8000"),
         _Step("SYNTH_WEBUI_TLS","","0"), _Step("TZ","","UTC")],
        group_e.items()
    )})
    ...
    # ── Group F: Database ─────────────────────────────────────────────────────
    _section("F. Database")
    _hint("Connection settings for MariaDB. Pre-filled from Stage 2 — only change if using a remote DB.")
    group_f = _run_steps([
        _Step("DB_HOST", "DB host", db_conf.get("DB_HOST", existing.get("DB_HOST", "127.0.0.1")),
              "127.0.0.1 for local MariaDB. Change if connecting to a remote server."),
        _Step("DB_PORT", "DB port", db_conf.get("DB_PORT", existing.get("DB_PORT", "3306")),
              "Default MariaDB port is 3306."),
        _Step("DB_USER", "DB user", db_conf.get("DB_USER", existing.get("DB_USER", "synth")),
              "The database user created in Stage 2."),
        _Step("DB_PASS", "DB password", db_conf.get("DB_PASS", existing.get("DB_PASS", "")),
              "Password for the synth DB user set in Stage 2."),
        _Step("DB_NAME", "DB name", db_conf.get("DB_NAME", existing.get("DB_NAME", "synth")),
    ...
    group_g = _run_steps([
        _Step("PROJECT_DEFAULT_LANGUAGE", "Default language", existing.get("PROJECT_DEFAULT_LANGUAGE", "en"),
              "ISO 639-1 language code, e.g. en, de, fr, ja. Used for generated content."),
        _Step("PROJECT_DEFAULT_TONE", "Default tone", existing.get("PROJECT_DEFAULT_TONE", "balanced"),
              "balanced / formal / casual / playful. Influences response style."),
        _Step("DIARY_HISTORY_DAYS", "Diary history days", existing.get("DIARY_HISTORY_DAYS", "7"),
              "How many days of diary entries to include in context. Higher = more memory, more tokens."),
        _Step("GRILLO_BEAT_INTERVAL", "Grillo beat interval (seconds)", existing.get("GRILLO_BEAT_INTERVAL", "900"),
              "How often Grillo autonomous beats run. 900 = 15 min. Set to 0 to disable."),
        _Step("ENABLE_RECON", "Enable recon (1/0)", existing.get("ENABLE_RECON", "0"),
              "Recon runs background research tasks. Set 1 to enable, 0 to disable (default off)."),
        _Step("ENABLE_DEBRIEF", "Enable debrief (1/0)", existing.get("ENABLE_DEBRIEF", "0"),
    ...
    group_h = _run_steps([
        _Step("CORTEX_API_LOG_ENABLED", "Enable cortex API log (1/0)", existing.get("CORTEX_API_LOG_ENABLED", "0"),
              "Logs every LLM request/response to cortex_api.log. Useful for debugging but grows fast. Default off."),
        _Step("LANGFUSE_ENABLED", "Enable Langfuse (1/0)", existing.get("LANGFUSE_ENABLED", "0"),
...
def stage_gitnexus(args: argparse.Namespace) -> bool:
    ...
    _hint("GitNexus indexes the codebase so Claude Code (and other MCP clients) can query it with context-aware searches.")
    if not shutil.which("npx"):
        _warn("npx not found — skipping GitNexus index. Install Node.js to enable.")
        return True
    if not args.non_interactive and not _confirm("Build GitNexus index now? (~1-2 min)", default=True):
        _ok("Skipped")
        return True
...
def stage_soul(args: argparse.Namespace, env_values: dict[str, str]) -> bool:
    ...
    _hint("Skip this now and re-run later: python scripts/windows_setup.py --stage soul")

    if args.non_interactive:
        _ok("Skipped (non-interactive mode)")
        return True
    if not _confirm("Enable SOUL long-term memory with Postgres/pgvector?", default=False):
        _ok("Skipped")
        return True
    ...
    _hint("The embedder converts text to vectors for semantic memory search. Larger = better quality but slower/heavier.")
    keys = list(EMBEDDERS.keys())
    for i, k in enumerate(keys, 1):
        spec: EmbedderSpec = EMBEDDERS[k]
        gpu_note = " [bold yellow][GPU recommended][/bold yellow]" if not spec.cpu_optimized else ""
        _print(f"    [cyan]{i}.[/cyan] [bold]{spec.display_name}[/bold] — {spec.dims}-dim, ~{spec.size_mb} MB{gpu_note}" if HAS_RICH
               else f"    {i}. {spec.display_name} — {spec.dims}-dim, ~{spec.size_mb} MB{'  [GPU recommended]' if not spec.cpu_optimized else ''}")
        _print(f"       [dim]{spec.description}[/dim]" if HAS_RICH else f"       {spec.description}")
    ...
    if not chosen_spec.cpu_optimized:
        _warn("This model is large and GPU-intensive. CPU inference will be very slow (~minutes per query).")

    _hint("DirectML works on any Windows GPU (NVIDIA, AMD, Intel) without CUDA. CUDA is for Linux NVIDIA only.")
    use_gpu = _confirm("Use GPU for embeddings? (DirectML on Windows, CUDA on Linux)", default=False)

    _hint("Format: postgresql://user:password@host:port/database")
    _hint("Example: postgresql://soul:soul@localhost:5432/soul_memory")
    dsn = env_values.get("SOUL_POSTGRES_DSN", "postgresql://soul:soul@localhost:5432/soul_memory")
    try:
    ...
    if SOUL_SQL_TEMPLATE.exists():
        sql = SOUL_SQL_TEMPLATE.read_text(encoding="utf-8").replace("{EMBEDDING_DIM}", str(chosen_spec.dims))
    ...
    _print(f"  Downloading {chosen_spec.display_name} model (~{chosen_spec.size_mb} MB)...")
    try:
        import os as _os
        ...
        _ok(f"Model downloaded to {REPO_ROOT / '.cache' / 'synth' / 'models' / 'fastembed'}")
    except Exception as exc:
...
def stage_service(args: argparse.Namespace) -> bool:
    ...
    if args.non_interactive:
        _ok("Skipped (non-interactive mode)")
        return True
    if not _confirm("Register SyntH as a Windows service (auto-starts on boot)?", default=False):
        _ok("Skipped")
        return True
    ...
    try:
        ...
        subprocess.run([nssm, "set", "SyntH", "AppDirectory", str(REPO_ROOT)], check=True)
        subprocess.run([nssm, "set", "SyntH", "Start", "SERVICE_AUTO_START"], check=True)
...
def stage_summary(results: dict[str, bool]) -> None:
    ...
    if HAS_RICH:
        ...
        t = RTable(show_header=True, header_style="bold magenta", border_style="magenta")
        t.add_column("Stage", style="cyan")
        t.add_column("Result")
        for stage, ok in results.items():
            t.add_row(stage, "[bold green] ✓ OK[/bold green]" if ok else "[bold red] ✗ FAILED / SKIPPED[/bold red]")
        console.print(t)
    else:
        for stage, ok in results.items():
    ...
    _print(f"\n[bold magenta]♥  SyntH is ready![/bold magenta]  WebUI → [link]http://localhost:{port}[/link]" if HAS_RICH
