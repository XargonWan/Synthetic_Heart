...
async def run(dry_run=True, delete_empty_content=False):
    ...
    for arch in archives:
        aid = arch.get('id')
        print(f'Inspecting archive {aid}: name={arch.get("name")} session={arch.get("session_id")} created={arch.get("created_at")} message_count_approx={arch.get("message_count")}')
        try:
            ...
            if isinstance(messages, list) and len(messages) == 0:
                ...
                if delete_empty_content:
                    all_empty = True
                    for msg in messages:
                        if isinstance(msg, dict) and ((msg.get('text') and str(msg.get('text')).strip()) or (msg.get('message_text') and str(msg.get('message_text')).strip())):
                            all_empty = False
                            break
                    if all_empty:
                        to_delete.append(aid)
...
def _main():
    parser = argparse.ArgumentParser(description='Delete chat archives with zero messages')
    parser.add_argument('--delete', action='store_true', help='Actually delete archives. Default: dry-run')
    parser.add_argument('--delete-empty-content', action='store_true', help='Also delete archives where messages exist but none contain text (or message_text)')
    args = parser.parse_args()
    # Use asyncio.run for modern event loop management
    try:
        rc = asyncio.run(run(dry_run=not args.delete, delete_empty_content=args.delete_empty_content))
        sys.exit(rc)
