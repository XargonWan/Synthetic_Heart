...
class DiscordInterface:
    ...
    async def _process_message(self, message):
        ...
        try:
            ...
            # Using channel_id (or thread_id if present) as the scope for attention state
            ...
            is_awake = self.chat_attention_state.get(chat_scope_id, False) # Default to Sleeping

            # Calculate explicit triggers
            is_explicit_trigger = is_wake_command or is_sleep_command
            if not is_explicit_trigger:
                if "@" in content:
                    is_explicit_trigger = True
                elif getattr(message, "guild", None) is None: # Private chat
                    is_explicit_trigger = True
                elif reply_to: # Reply to bot?
                     # reply_to is a SimpleNamespace with from_user
                     bot_user = getattr(self.client, "user", None)
                     if bot_user and getattr(reply_to.from_user, "id", None) == bot_user.id:
                         is_explicit_trigger = True

            # Check if message is for bot using core utility
            ...
            try:
                await message_queue.enqueue(self.client, wrapped, interface_id="discord_bot", original_message=message, skip_mention_check=True)
