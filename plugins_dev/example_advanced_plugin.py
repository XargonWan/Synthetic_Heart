...
class ExampleAdvancedPlugin:
    ...
    def _register_custom_validation(self):
        ...
        try:
            ...
            def validate_api_call(payload):
                ...
                if method in ["POST", "PUT", "DELETE"] and not payload.get(
                    "auth_token"
                ):
            ...
            log_warning(
                f"[example_advanced_plugin] Failed to register custom validation: {e}"
            )
    ...
    async def run_action(self, action_type: str, payload: dict, context: dict = None):
        ...
        log_info(
            f"[example_advanced_plugin] Executing {action_type} with payload: {payload}"
        )
