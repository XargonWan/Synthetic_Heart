...
2. Check for missing dependencies in ``pyproject.toml`` (use `uv sync` to install).
   * Note: ``kitten`` is vendored directly and requires no external
     repository.  If a build fails it usually means a third-party dependency
     could not be compiled.  Adding the necessary system packages to the
     Dockerfile or fixing the upstream repo is the correct remedy; do not
     attempt to manually ``pip``-install inside the container.
