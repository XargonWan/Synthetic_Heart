...
The **OpenAI-compatible API server** exposes Synthetic Heart through a standard OpenAI-style HTTP surface. For backward compatibility it also mirrors the legacy REST API used by `ollama <https://github.com/ollama/ollama>`_, so any tool that can talk to an OpenAI endpoint—or to a local Ollama daemon—can point at SyntH and receive the responses generated by your active persona.
